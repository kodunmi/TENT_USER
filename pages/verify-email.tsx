...
const VerifyEmail = () => {
  ...
  const [resendMail, { isLoading: sendingMail }] =
    useResendVerificationMailMutation();
  const [sendOtpToPhone, { isLoading: sendingOTP }] =
    useRequestPhoneVerificationMutation();
  ...
  const verifyEmail = async () => {
    try {
      ...
      let verifyPhone = await sendOtpToPhone(phoneWithCountryCode).unwrap();

      enqueueSnackbar(
        res.message ? res.message : "We could not process your request",
        {
          variant: "success",
        }
      );

      enqueueSnackbar(verifyPhone.data, {
        variant: "success",
  ...
  const resendMailFunc = async () => {
    try {
      ...
      console.log(res);
      enqueueSnackbar(
        res.data ? res.data : "We could not process your request",
        {
          variant: "success",
        }
      );
      ...
      <Box style={{ position: "relative" }}>
        ...
        <Box px={3} pb={2}>
          ...
          <Typography align="center" component="div" mt={3} variant="caption">
            ...
            <Button variant="text" onClick={resendMailFunc}>
              Resend
            </Button>
          </Typography>
        </Box>
