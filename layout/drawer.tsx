            ...
            {MenuList.map((menu) => (
              <StyledNavLink key={`navlink ${menu.route}`} href={menu.route}>
                <StyledListItem
                  ...
                  button
                  key={menu.route}
                  secondaryAction={menu.countable ? <MenuCount>{ menu.route === "/notifications" ? 12 : menu.route == "/orders" ? 12 : 6} </MenuCount> : menu.route === "/profile" ? user.accountVerified ? <Activation background="green">ACTIVATED</Activation>:<Activation background="red">UNACTIVATED</Activation> : ""}
                >
                  <ListItemIcon>
