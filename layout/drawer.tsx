...
export const SideDrawer = ({
  ...
  const { refetch, data, isLoading, error } = useGetMyOrdersQuery(
    { pageNumber: 1, sortBy: "createdAt", order: "desc" },
    {
                  ...
                  menu.countable ? (
                    <MenuCount>
                      {menu.route === "/notifications"
                        ? 12
                        : menu.route == "/orders"
                        ? !error && !isLoading && data.data.myOrderCount
                        : 6}{" "}
                    </MenuCount>
                  ) : menu.route === "/profile" ? (
                    user.user.profileVerified ? (
                      <Activation background="green">ACTIVATED</Activation>
                    ) : (
                      <Activation background="red">UNACTIVATED</Activation>
                    )
                  ) : (
                    ""
                  )
